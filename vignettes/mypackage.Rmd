...
colnames(fit_act) <- c("fit", "actual") # rename columns
...
Based on cv error I would choose k_nn = 1 since it seems to output the lowest error.

Based on testing error I would choose k_nn = 2 since choosing k_nn = 1 is meaningless.

In practice, we would choose k_nn = 2 since we are minimizing our risk to underfitting but also overfitting.

We use cross validation to randomly split data to compare to each other, this way, we are able to gauge the performance of our estimating by analyzing the errors! In particular, we use cross validation to minimize overfitting as we are able to use multiple sets of training data within the dataset to create the best model.
